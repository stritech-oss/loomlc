...
# A header starting with "Merge " is not a free pass: CI already skips real merge commits, so this arm
# only ever sees ordinary ones, which still owe a sign-off.
expect fail "merge-looking header without a sign-off" msg "Merge the two config loaders and drop validation.\n" "missing Signed-off-by"
expect pass "merge commit with a sign-off" msg "Merge branch 'main' into feat/x\n\n$sob\n"
expect fail "agent footer without the generated words" msg "fix(cli): handle empty task ref\n\n$sob\n🤖 [Claude Code](https://claude.ai/code)\n" "agent attribution"
expect fail "session trailer with a suffix" msg "fix(cli): handle empty task ref\n\nClaude-Session-Id: 4f2a9e10\n$sob\n" "agent attribution"
expect fail "thread trailer" msg "fix(cli): handle empty task ref\n\nCodex-Thread: https://example.invalid/t/1\n$sob\n" "agent attribution"
expect fail "generated-by trailer" msg "fix(cli): handle empty task ref\n\n$sob\nGenerated-by: Claude Code\n" "agent attribution"
expect fail "assisted-by trailer" msg "fix(cli): handle empty task ref\n\nAssisted-by: Claude <noreply@anthropic.com>\n$sob\n" "agent attribution"
# loomlc's own docs describe agents generating things constantly; only a footer is attribution.
expect pass "prose about an agent generating output" text "The change step is generated by claude, then checked by qa.\n"
expect pass "prose naming a provider's site" text "Schemas are generated by pi.dev adapters at build time.\n"
